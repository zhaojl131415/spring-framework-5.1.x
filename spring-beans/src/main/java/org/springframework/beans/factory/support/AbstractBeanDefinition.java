...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * 当前bean创建时必须要依赖哪个bean先被创建
	 */
	@Nullable
	private String[] dependsOn;

	/**
	 * 设置为false的时候，这样容器会在自动装配对象的时候，会不考虑当前bean
	 * 他不会被看作其他bean依赖的bean，但是他依赖其他bean是能够被自动装配进来的
	 */
	private boolean autowireCandidate = true;

	/**
	 * 当发生自动装配时，假设某个bean发现多个，那么标注@Primary为true的首先被注入
	 */
	 ...
	 * bean definition.
	 * 创建一个新的AbstractBeanDefinition作为给定bean definition的深度副本。
